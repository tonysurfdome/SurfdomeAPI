...
class ReturnStock{
	...
	public function getData() {
		...
		$sql = "SELECT 
					TOP 100 
						it.ItemCode 
						,max(CONVERT(CHAR(19),h.DATETIMESTAMP,120)) as datetimestamp 
					FROM 
					...
					group by it.ItemCode 
						having 
						(max (CONVERT(CHAR(19),h.DATETIMESTAMP,120)) > '".$this->dateTimeStamp."')  
						or 
						(max(CONVERT(CHAR(19),h.DATETIMESTAMP,120)) ='".$this->dateTimeStamp."' AND	it.ItemCode > '".$this->itemCode."')
					order by max(CONVERT(CHAR(19),h.DATETIMESTAMP,120)), ItemCode";
