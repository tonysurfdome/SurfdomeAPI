...
$sql ="SELECT count(*)
  FROM [ActionGroup]
  where  [ActionGroup].ActionTypeId =2 and CompletedTimestamp is null";
...
$sql = "select COUNT(actiongroupid)   from actiongroup WHERE 
	[ActionGroup].ActionTypeId =2 and CompletedTimestamp is null and LockedByUserId is not null";

$arr_data['lockedbatches'] = ms_query_value($sql);


$sql = "select COUNT(actiongroupid)   from actiongroup WHERE 
	  [ActionGroup].ActionTypeId =2 and CompletedTimestamp is null and LockedByUserId is  null";
...
MAX([USER].DisplayName) as DisplayName
  ...
  WHERE 
	  [ActionGroup].ActionTypeId =2 and CompletedTimestamp is null ".$Priority  .$locked. "
  GROUP BY 
   ActionGroup.Name
