<?php
	...
	if (isset($_REQUEST['mode']) && $_REQUEST['mode'] == "getData")
	{
		$date =  date('Y-m-d',strtotime("yesterday"));
			...
			$sql = "SELECT 
						CONVERT(CHAR(16),so.RequestedDeliveryDate,120) as RequestedDeliveryDate ,
						so.SalesOrderNumber,
						so.CustomerPurchaseOrderReferenceNumber as webnum,
						sos.name as status,
						so.ChannelName,
					...
					FROM 
					 ...
					 WHERE 
						 so.statusid =8
						 and 
						 so.SalesOrderNumber like 'ST%'
						 and
						 so.SalesOrderNumber not LIKe 'STUDIO%' 
						 and  
							 soip.QuantityPicked =0
						and
						CONVERT(CHAR(16),so.RequestedDeliveryDate,120) <= '".$date." 17:00' 
					ORDER BY 
						CONVERT(CHAR(16),so.RequestedDeliveryDate,120) DESC";
